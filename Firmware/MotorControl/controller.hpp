...
public:
    ...
    struct Config_t {
        ...
        float pos_gain = 1.0f;                  // [(turn/s) / turn]
        float vel_gain = 0.001f;            // [Nm/(turn/s)]
        // float vel_gain = 0.2f / 200.0f,       // [Nm/(rad/s)] <sensorless example>
        float vel_integrator_gain = 0.003f; // [Nm/(turn/s * s)]
        float vel_limit = 250.0f;                  // [turn/s] Infinity to disable.
